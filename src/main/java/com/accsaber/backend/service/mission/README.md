...
- **STREAK_ON_MAP** - streak target built from the user's *per-complexity-band* representative streak, not one blended category number. The map is sampled first, its complexity resolves to a 3-wide half-open band (`[1,4) [4,7) [7,10) [10,13) [13,16)`), and we pull the user's top streaks **on maps in that band only** (`representativeUserStreakForComplexityBand`). That `bandAbility` sets a difficulty-shaped cap, so a hard map is measured against how the user streaks on hard maps - not inflated by their easy-map streaks. Reference = `0.6*avg + 0.4*max` of the map's own top-5 streaks; a ranked map always has imported scores, so `bandAbility` is only a defensive fallback if that leaderboard ever comes back empty. Final demand = `streakTargetFor(band)` clamped to at least 3 and at most `bandAbility + 1` for easy, `bandAbility - 1` for medium, `bandAbility` for hard, or `bandAbility + 2` for extreme. The `< 3` gate now runs **after** the pick on `bandAbility`, inside a `COMPUTE_MAP_RETRIES` resample loop: if the sampled map lands in a band the user can't streak 3 on, we resample rather than drop the mission. If the user has no streak data in the band at all, `bandAbility` falls back to their category-wide representative streak translated down by `complexityTranslationFactor` (so an easy-map streak isn't reused as-is on a hard map).
- **STREAK_N_IN_CATEGORY** - same streak logic, plus a count. Routes through the shared `streakTargetFor`; extreme always gets `+1` on the streak.
- **STREAK_SUM_N** - **event-only** (see the event-only block below). Every submitted score adds its `streak115` to `progressCount` until it reaches `targetCount` - no per-map dedupe, and partials count, so attempts are included by design. It's the only type that sums a score field other than XP. Be aware the additive count is grindable on one easy map, which is fine for a fixed-target event mission but would need a per-map cap for a calibrated pool.
- **SNIPE_RIVAL_ANY_MAP** - **event-only**. Nothing like `SNIPE_PLAYER_ON_MAP`: no frozen target, no map pick. On each active score it asks `existsRivalScoreBelow(mapDifficulty, rivalIds, score)` and counts up to `targetCount`. Rival ids come from `user_relations` (type `rival`, outgoing only) and are resolved **once per score** via `EvalContext`, not once per mission - `evaluateAllForUser` loops every active mission and rival lists are uncapped. No skill gate and no per-map dedupe, so it's trivially satisfiable against a weak rival, and it counts "scored above a rival" rather than strictly "was below, now above"; fine for a fixed-XP `targetCount = 1` event mission, not for a calibrated one.
- **AP_GAIN_OVERALL** - **event-only**. Accumulates into `progressAp` (NUMERIC) against `targetAp`; `progressCount` is an INTEGER and can't hold fractional AP. The gain per score is `active_overall_row.ap - supersedes.ap` via `findActiveApGainOverPrevious`. **The `score.isActive()` guard is load-bearing**: a worse score still fires `ScoreSubmittedEvent` but triggers no stats recalc, so without the guard we'd re-read the same unchanged row and credit the *previous* score's gain again, forever. Reweights are immune by construction - `ScoreRecalculationService` mints new AP versions without firing the event - which is why this is incremental rather than a snapshot diff against `assignedAt`.
- **BATCH_PLAY_N** - **event-only**. Resolves the latest `released` batch at *evaluation* time (not assignment), memoized per score in `EvalContext`, then checks the played difficulty belongs to it. Deliberate: over a 4-week event "the latest batch" should follow the newest release. Progress already banked on a previous batch stays banked. Counts partials, like `PLAY_N_MAPS` - it's a "play" mission, not a "score" one.
- **PB_RANKED_BEFORE_N** - **event-only**. Counts active scores on difficulties whose `rankedAt` is before `targetRankedBefore`. That's the AccSaber *ranking* date, not the BeatSaver upload date - we don't store the latter (the field isn't even on `BeatSaverMapResponse`, and `@JsonIgnoreProperties` drops it), so "old song" missions are not expressible today. `rankedAt` is also `now()`-defaulted at import, so it's noisy for legacy rows.
- **CAMPAIGN_COMPLETE_N** - **event-only, and the only non-score-triggered type.** Driven by `CampaignCompletedEvent`, published from inside `CampaignEvaluationService.evaluateCampaignCompletion`'s `newlyCompleted` guard - that one site covers all five paths that can complete a campaign (score submit, curated transition, legacy import, bulk settle), and the guard stops it double-firing on the uncurated→curated re-entry. `targetCuratedOnly` filters on `campaign.status == CURATED`; leave it null for "any campaign". Note campaign completion is **reversible** (`recomputeAfterRequirementChange` demotes to IN_PROGRESS), so the same user can legitimately re-complete and re-fire.
...
- **`representativeUserStreak` outlier rule** - if `max > median * 1.5`, switch to `median * bandMultiplier`. Without this, one fluke 115-streak permanently inflates streak missions. `max`/`median` are read from the top `STREAK_OUTLIER_WINDOW` (10) values only, so widening the sample below does not shift this rule.

- **`representativeUserStreak` sample depth** - the query pulls the top `STREAK_SAMPLE_SIZE` (30) streaks, and the non-outlier reference is one band-agnostic pick at `STREAK_REFERENCE_DEPTH` (0.30) of the list, floored at index `STREAK_MIN_DEPTH_INDEX` (4). A top-10 window could only ever return a streak the player hit a handful of times ever - for a player whose 10 best streaks all sit within a 2-note span, every index returned the same number, which is what made these missions feel unmovable. The floor keeps players with fewer than 10 recorded streaks on exactly the pre-existing `hard` reference; band shaping lives solely in `streakTargetFor`.
- **`streakTargetFor` (0.5/0.7/0.9/1.0)** - streak target vs map reference. Hard gets a `+1` for top-tier players; extreme always gets `+1`.
